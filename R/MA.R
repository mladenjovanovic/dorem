# ------------------------------------------------------------------------------
# Training function
MA_train <- function(predictors, outcome, control = dorem_control()) {
  ...
  predict_func <- function(par, predictors) {
    ...
    coefs <- MA_make_coefs(predictors)

    # Convert par to coefs again
    coefs <- MA_vector_to_coefs(values = par, coefs = coefs)

    # Predict
    model <- list(coefs = coefs, control = control)
    predicted <- MA_predict(model, predictors)
  ...
  # -------------------------------------
  # Get coefs
  coefs_start <- MA_coefs_start(predictors, outcome, control$na.rm)
  coefs_lower <- MA_coefs_lower(predictors, outcome, control$na.rm)
  coefs_upper <- MA_coefs_upper(predictors, outcome, control$na.rm)

  # Check if user provided starting coefs
  if (!is.null(control$coefs_start)) {
    coefs_start <- MA_vector_to_coefs(control$coefs_start, coefs_start)
  }

  # Check if user provided coefs lower bound
  if (!is.null(control$coefs_lower)) {
    coefs_lower <- MA_vector_to_coefs(control$coefs_lower, coefs_lower)
  }

  # Check if user provided coefs upper bound
  if (!is.null(control$coefs_upper)) {
    coefs_upper <- MA_vector_to_coefs(control$coefs_upper, coefs_upper)
  }

  # Convert coefs to par
  control$coefs_start <- MA_coefs_to_vector(coefs_start)
  control$coefs_lower <- MA_coefs_to_vector(coefs_lower)
  control$coefs_upper <- MA_coefs_to_vector(coefs_upper)
  ...
  # Extract coefs
  coefs_result <- MA_vector_to_coefs(opt_res$par, coefs_start)
...
MA_predict <- function(model, predictors) {
  coefs <- model$coefs
  ...
  training_responses <- purrr::map2(
    ...
    .f = function(.x, .y) {
      ...
      PTE <- .x$PTE_gain * TTR::SMA(x = .y, n = as.integer(.x$PTE_tau))
      NTE <- .x$NTE_gain * TTR::SMA(x = .y, n = as.integer(.x$NTE_tau))
      return(PTE - NTE)
    }
  )
...
# --------------------------------------------------------------------------------------------
MA_make_coefs <- function(predictors) {
...
MA_coefs_start <- function(predictors, outcome, na.rm = TRUE) {
  coefs <- purrr::map(predictors, function(...) {
    list(
...
# --------------------------------------------------------------------------------------------
MA_coefs_lower <- function(predictors, outcome, na.rm = TRUE) {
  coefs <- purrr::map(predictors, function(...) {
    list(
      PTE_gain = 0, PTE_tau = 1,
      NTE_gain = 0, NTE_tau = 1
    )
  })
...
# --------------------------------------------------------------------------------------------
MA_coefs_upper <- function(predictors, outcome, na.rm = TRUE) {
  coefs <- purrr::map(predictors, function(...) {
    list(
      PTE_gain = 10000, PTE_tau = 42,
      NTE_gain = 10000, NTE_tau = 42
    )
  })
...
# ---------------------------------------------------------------------
MA_coefs_to_vector <- function(coefs) {
  unlist(coefs)
}

# ---------------------------------------------------------------------
MA_vector_to_coefs <- function(values, coefs) {
  coefs_new <- coefs
